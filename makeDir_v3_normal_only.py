...
output = open("toRun_normal.sh","w");
output1 = open("toRun_normal_tmux.sh","w");
output2 = open("missingSamples_notnormal.txt","w");
...
#	subprocess.call("ln -s "+pathDict[sample]["T"]["R2"]+" "+outputFolder+"/"+sample+"_T/"+sample+"_T_2.fastq.gz",shell=True);
	...
	print("bash /diskmnt/Projects/cptac_scratch/CPTAC3_analysis/Fusion_hg38/EJ_testing/fusion_pipeline_v2.sh "+sample+"_N "+sample+"_N_1.fastq.gz "+sample+"_N_2.fastq.gz 4",file=output);
	print("tmux new -d -s "+ sample +" '"+ "bash /diskmnt/Projects/cptac_scratch/CPTAC3_analysis/Fusion_hg38/EJ_testing/fusion_pipeline_v2.sh "+sample+"_N "+sample+"_N_1.fastq.gz "+sample+"_N_2.fastq.gz 4" +"'",file=output1);
